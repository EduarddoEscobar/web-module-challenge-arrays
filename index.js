...
function getAverageWordLength(arr){
  let count = 0;
  arr.forEach(element => count+=element.split(' ').length);
...
function getRandomFlavors(arr1, arr2, arr3, arr4){
  const possibleArrays = [arr1, arr2, arr3, arr4];
  let randomFlavors = [];
  for(let i=0;i<31;i++){
    let arrayChosen = possibleArrays[Math.floor(Math.random() * possibleArrays.length)];
    randomFlavors.push(arrayChosen[Math.floor(Math.random() * arrayChosen.length)]);
  }
